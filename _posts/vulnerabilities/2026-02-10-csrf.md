...
1. La víctima debe estar **autenticada** en la aplicación.  
2. El servidor debe enviar automáticamente las **cookies** de sesión del usuario.  
3. Debe existir en el servidor algún **vector vulnerable** que permita ejecutar la acción no deseada.
...
Abrimos dos pestañas y, tras interceptar una petición de cambio de correo para cada usuario, comprobamos lo siguiente:
...
El proceso para resolver este laboratorio es exactamente el mismo que en el [Lab 5](#lab-5), solo que en esta ocasión la **cookie `csrfkey`** y el **token CSRF** son idénticos.  
...
- La aplicación permite cambios de email mediante `GET`.  
...
Esto nos hace intuir que el servidor realiza algún tipo de validación del **origen**, pero no de manera estricta; parece que permite variaciones, posiblemente para **subdominios** u otras modificaciones menores en la URL.
Podemos irnos al **exploit server** y, en lugar de hospedar el código malicioso bajo la ruta `/exploit` como hemos hecho siempre, podemos, por ejemplo, usar como ruta el **mismo nombre de la URL del servidor víctima**.  
