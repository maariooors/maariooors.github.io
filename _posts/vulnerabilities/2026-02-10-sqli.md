...
Los principales tipos de inyección SQL incluyen: [inyección directa](#inyección-directa), [inyección a ciegas](#inyección-a-ciegas), [inyección basada en errores](#inyección-basada-en-errores) e [inyección fuera de banda (out-of-band)](#inyección-fuera-de-banda-out-of-band).
...
- [Las bases de datos existentes](#enumeración-de-las-bases-de-datos-existentes).
- [Las tablas contenidas en cada base de datos](#enumeración-de-tablas-existentes).
- [Las columnas presentes en dichas tablas](#enumeración-de-columnas-existentes).
...
- [Inyección basada en condicionales](#inyección-basada-en-condicionales).
- [Inyección basada en tiempo](#inyección-basada-en-tiempo).
...
La inyección fuera de banda (Out-of-Band, OOB) se presenta en los casos en los que, mediante ninguno de los métodos anteriores, es posible obtener información directamente en la respuesta del servidor. Su funcionamiento consiste en lograr que la víctima, a través de un campo vulnerable, realice un **`DNS Lookup`** hacia un dominio controlado por el atacante, lo que permite exfiltrar información de manera indirecta.

Para esta demostración utilizaremos Burp Suite Professional, lo que nos permite aprovechar los laboratorios de PortSwigger gracias a la funcionalidad de Collaborator.

En este laboratorio, el campo vulnerable es el mismo que en las inyecciones a ciegas: un parámetro **`TrackingId`** en el que no se valida correctamente el input del usuario. El primer paso para explotar este vector de ataque es comprobar si podemos recibir una conexión desde la máquina víctima. Esto se consigue con el siguiente payload:
```bash
...
Si a continuación nos dirigimos a la sección Collaborator en Burp Suite y revisamos las solicitudes recibidas, podemos observar lo siguiente:

<div style="text-align: center;">
  <img src="{{ site.baseurl }}/assets/sqli/sqli-1.png" width="1000"/>
</div>

Con esta información confirmamos que el campo es vulnerable y que la víctima puede enviarnos datos. Por ello, empleamos el siguiente payload para extraer información:
```bash
...
Si a continuación volvemos a revisar el Collaborator, podemos observar lo siguiente:

<div style="text-align: center;">
  <img src="{{ site.baseurl }}/assets/sqli/sqli-2.png" width="1000"/>
</div>

Como se puede observar, hemos recibido una petición hacia un supuesto subdominio bajo nuestro control, donde la primera parte **`r8guco2cc08lrtn10us5`** corresponde realmente a la contraseña del usuario administrador.  

Es importante destacar que, para realizar este tipo de inyección, es necesario suponer tanto el nombre de la tabla como el de las columnas, lo que añade un grado adicional de complejidad al ataque.